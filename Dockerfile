...
RUN python3 -m venv /venv && \
    ...
    /venv/bin/pip install --no-cache-dir sageattention==1.0.6 triton==3.5.0
...
# --------------------------
# 🧪 Post-start: sageattention + triton again if needed
# --------------------------
RUN /venv/bin/pip install --no-cache-dir --upgrade triton ninja && \
    /venv/bin/pip install --no-cache-dir --no-binary=:all: sageattention

# --------------------------
# 🧰 Scripts + Entrypoint + Binaries
# --------------------------
