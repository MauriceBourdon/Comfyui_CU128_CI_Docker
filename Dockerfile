...
# Python env + PyTorch cu128 (torch only to avoid conflicts/size)
...
COPY scripts/restore_state.py /scripts/restore_state.py
...
# Defaults
ENV ENABLE_JUPYTER=true     JUPYTER_PORT=8888     COMFY_AUTOSTART=true     COMFY_PORT=8188     COMFY_ARGS="--listen 0.0.0.0 --port 8188 --use-sage-attention"     DATA_DIR=/workspace     COMFY_DIR=/opt/ComfyUI     COMFY_PERSIST=true     COMFY_HOME=/workspace/ComfyUI     MODELS_DIR=/workspace/models     MODELS_MANIFEST=/workspace/models_manifest.txt     POST_START_ENABLED=true     PIP_CACHE_DIR=/workspace/.pip-cache     PIP_NO_CACHE_DIR=0
